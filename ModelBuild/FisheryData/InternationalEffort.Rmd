...
E_PL <- read_csv(here("ModelBuild", "FisheryData", "PublicLPTuna",
                      "PublicLPTunaFlag.csv"), show_col_types = FALSE)
```
...
north <- str_detect(W_PL$LAT5, "N")
W_PL$LAT5[north] <- str_sub(W_PL$LAT5[north], end = -2)
south <- str_detect(W_PL$LAT5, "S")
W_PL$LAT5[south] <- str_sub(W_PL$LAT5[south], end = -2)
east <- str_detect(W_PL$LON5, "E")
W_PL$LON5[east] <- str_sub(W_PL$LON5[east], end = -2)
west <- str_detect(W_PL$LON5, "W")
W_PL$LON5[west] <- str_sub(W_PL$LON5[west], end = -2)
W_PL <- W_PL |>
  mutate_at(c('LAT5', 'LON5'), as.numeric)
W_PL$LAT5[south] <- W_PL$LAT5[south] * -1
W_PL$LON5[west] <- 360 - W_PL$LON5[west]
rm(north, south, east, west)

# Convert to 360 longitude
E_LL$LonC5 <- E_LL$LonC5 + 360
E_PS$LonC1 <- E_PS$LonC1 + 360
E_PL$LonC1 <- E_PL$LonC1 + 360
...
W_PL_intl_effort <- W_PL_intl |>
  select(LAT5, LON5, DAYS) |>
  group_by(LAT5, LON5) |>
  summarise(TotalEffort = sum(DAYS))

E_PL_intl_effort <- E_PL_intl |>
  select(LatC1, LonC1, NumSets) |>
  group_by(LatC1, LonC1) |>
  summarise(TotalEffort = sum(NumSets))
...
W_PL_intl_effort$LAT5 <- W_PL_intl_effort$LAT5 + 2.5
W_PL_intl_effort$LON5 <- W_PL_intl_effort$LON5 + 2.5
...
W_PL_intl_effort <- W_PL_intl_effort |>
  rename(Lat = LAT5, Lon = LON5)
E_PL_intl_effort <- E_PL_intl_effort |>
  rename(Lat = LatC1, Lon = LonC1)
...
Intl_PL_map <- ggplot() + 
  geom_tile(data = W_PL_intl_effort, aes(x = Lon, y = Lat, fill = TotalEffort)) + 
  geom_tile(data = E_PL_intl_effort, aes(x = Lon, y = Lat, fill = TotalEffort)) +
  coord_quickmap(c(150, 250), ylim = c(-25, 50)) + 
  annotation_borders("world2", fill = "#A5AAAF")
...
# Save 
# png(here("ModelBuild", "FisheryData", "Intl_PL_map.png"), bg = "transparent")
# Intl_PL_map
# dev.off()
```
