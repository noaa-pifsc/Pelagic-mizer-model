...
oceanographically defined model domain boundaries, together with Hawaiʻi-based
...
# Remove leading LLs from trip number and convert to numeric
...
ObsSets <- ObsSets |>
  mutate(YEAR = year(HAUL_BEGIN_DATETIME),
         ...
         Quarter = NA,
         Trip_Numeric = as.numeric(str_sub(TRIP_NUM, 3, 7)),
...
# Restrict ourselves to trip 2060+
# First trip when the every-third-fish measurement protocol was implemented
# Latest full year
Obs2060 <- ObsSets |>
  # filter(Trip_Numeric >= 2060)
  filter(YEAR >= 2015)

# Convert to 360-deg longitude
neg_lon <- which(Obs2060$HAUL_BEGIN_LON < 0)
Obs2060$HAUL_BEGIN_LON[neg_lon] <- Obs2060$HAUL_BEGIN_LON[neg_lon] + 360

# Fill Quarter column
# I'm sure there's a tidy way to do this, but it's escaping me
Q1 <- which(Obs2060$MONTH < 4)
Q2 <- which(Obs2060$MONTH > 3 & Obs2060$MONTH < 7)
Q3 <- which(Obs2060$MONTH > 6 & Obs2060$MONTH < 10)
Q4 <- which(Obs2060$MONTH > 9)
Obs2060$Quarter[Q1] <- 1
Obs2060$Quarter[Q2] <- 2
Obs2060$Quarter[Q3] <- 3
Obs2060$Quarter[Q4] <- 4

# Filter to shallow sets only
Obs2060_Shallow <- Obs2060 |>
  filter(HKS_PER_FLT < 15)

# Filter to shallow sets only
Obs2060_Deep <- Obs2060 |>
  filter(HKS_PER_FLT >= 15)
```
...
ocean_domain_map <- ggplot() +
  geom_point(data = Obs2060_Deep, aes(x = HAUL_BEGIN_LON, y = HAUL_BEGIN_LAT),
             shape = 21, color = "#003087") +
  geom_point(data = Obs2060_Shallow, aes(x = HAUL_BEGIN_LON, y = HAUL_BEGIN_LAT),
  ...
  geom_line(aes(x = c(180, 180), y = c(-25, 50)), color = "black") + 
  coord_quickmap(c(150, 250), ylim = c(-25, 50)) + 
  annotation_borders("world2", fill = "#A5AAAF")
...
# pdf(here("ModelBuild", "OceanographicData", "ocean_domain_map_w2p1.pdf"))
# ocean_domain_map
# dev.off()
...
ocean_domain_map_Q1 <- ggplot() +
  geom_point(data = Obs2060_Deep_Q1, aes(x = HAUL_BEGIN_LON, y = HAUL_BEGIN_LAT),
             shape = 21, color = "#003087") +
  geom_point(data = Obs2060_Shallow_Q1, aes(x = HAUL_BEGIN_LON, y = HAUL_BEGIN_LAT),
             shape = 21, color = "#5EB6D9") +
...
ocean_domain_map_Q2 <- ggplot() +
  geom_point(data = Obs2060_Deep_Q2, aes(x = HAUL_BEGIN_LON, y = HAUL_BEGIN_LAT),
             shape = 21, color = "#003087") +
  geom_point(data = Obs2060_Shallow_Q2, aes(x = HAUL_BEGIN_LON, y = HAUL_BEGIN_LAT),
             shape = 21, color = "#5EB6D9") +
...
ocean_domain_map_Q3 <- ggplot() +
  geom_point(data = Obs2060_Deep_Q3, aes(x = HAUL_BEGIN_LON, y = HAUL_BEGIN_LAT),
             shape = 21, color = "#003087") +
  geom_point(data = Obs2060_Shallow_Q3, aes(x = HAUL_BEGIN_LON, y = HAUL_BEGIN_LAT),
             shape = 21, color = "#5EB6D9") +
...
ocean_domain_map_Q4 <- ggplot() +
  geom_point(data = Obs2060_Deep_Q4, aes(x = HAUL_BEGIN_LON, y = HAUL_BEGIN_LAT),
             shape = 21, color = "#003087") +
  geom_point(data = Obs2060_Shallow_Q4, aes(x = HAUL_BEGIN_LON, y = HAUL_BEGIN_LAT),
             shape = 21, color = "#5EB6D9") +
