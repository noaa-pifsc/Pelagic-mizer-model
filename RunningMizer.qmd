...
mizer in order to extract the size bins.  All this is to say, while we present 
things here as a straightforward unidirectional workflow, know that building and 
working with your own model will be iterative.
...
track the references we used.  It also allowed us to easily [add](https://github.com/noaa-pifsc/Pelagic-mizer-model/blob/main/ModelBuild/FisheryData/CatchSpectra.Rmd) 
them to the gear parameters. We're going to remove that column now, just to 
avoid any possible confusion down the line.
```{r}
...
parameters that mizer will use to run simulations.  To look at the species 
parameters, for example, use `pelagic_params@species_params`.  You can use 
similar syntax to take a look at the other parameters.  The 
[mizer site](https://sizespectrum.org/mizer/index.html) offers extensive 
descriptions of the parameters and discusses this function in detail.  It's 
worth reading carefully to better understand this step.
...
model in @fig-steady-spectra.
```{r}
#| message: false
#| label: fig-steady-spectra
#| fig-cap: Steady-state model spectra (note that the "Total" line mostly covers up the "Resource" line)
#| fig-alt: Biomass spectra that suggest the model is performing reasonably
...
Note that when running to a steady state, mizer recalculates some 
parameters.  To see what parameters are being used you can 
take a similar approach to that used above (`pelagic_steady@species_params`).
...
![Summary plot for a mizer simulation](OverviewContent/sim_fig.png){#fig-sim alt="five-paneled figure showing feeding level, biomass, predation mortality, fishing mortality, and biomass spectra for each of the modeled species"}
